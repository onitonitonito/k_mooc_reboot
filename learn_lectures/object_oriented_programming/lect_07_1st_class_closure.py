...
_="""4. 클로저함수 : 다른 변수에 함수를 할당했기 때문에, 지워지지 않음.
 (1) 외부함수(2차함수)에서 내부함수(1차함수)의 바디를 반환한다.
 (2) 변수에 2차함수()를 할당 하면, 반환값 내부함수의 바디가 저장 됨.
 (3) 변수가 외부함수의 역할을 대신 수행 하면서, 내부값을 기억하고 있음.

 보통 '클로저'는 '람다'와 함께 사용하는 경우가 많아 둘을 혼동하기 쉽습니다.
 '람다'는 이름이 없는 익명 함수를 뜻하고, '클로저'는 함수를 둘러싼 환경을
 유지했다가 나중에 다시 사용하는 함수
"""
print('\n\n'+_)
def counter():
    i = 0
    def count():
        nonlocal i          # 고차함수에서 'global' 과 같은 역할수행
        i += 1
        return i
    return count            # 중요! 내부함수()의 바디를 반환한다
...
print(c())          # 3
print(c())          # 4
# del(counter)      # Error : Not exists
print('del(counter)      # Error : Not exists\n')

print(c())          # 5
print(c())          # 6 .. 내부변수 i를 계속 가져 감.
